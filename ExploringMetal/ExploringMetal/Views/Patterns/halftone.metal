...
/// Calculate the signed distance field of a circle (SDF)
/// How far is the `position` from the perimeter of a circle at `center` with a radius of `r`.
/// Positive ? `position` lies outside the circle
/// Zero ? `position` lies on the perimeter of the circle
/// Negative ? `position` lies inside the circle
/// SDF is a powerful tool when drawing with shaders
/// Look at https://iquilezles.org/articles/distfunctions2d/ for more formulas for 2D
